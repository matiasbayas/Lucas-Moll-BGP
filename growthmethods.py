...
def updateW(w0, f0, F0, gamma, xgrid, deltax, I, rho, theta, k, eta, alpha0, xi,  maxit_v, tol_v):
    ...
    for it in range(maxit_v):
        ...
        B = np.zeros([I,I])
        for j in range(1,I):
            B[j,j] = rho - gamma + alpha[j] * (1-F0[j]) + gamma * xgrid[j] / deltax[j] - np.sum(alpha[0:j]*f0[0:j]*deltax[0:j])
            B[j,j-1] = - gamma * xgrid[j] / deltax[j]
...
def updateDist(gamma, alpha, xgrid, deltax, I, rho, theta, k, eta, alpha0, xi, maxit_g, tol_g):
    ...
    for it_g in range(maxit_g):
        ...
        # Update gamma:
        gammanew = xi*(theta *( alpha.T@(f*deltax) + alpha[I-1]*(1-F[I-1]) + alpha[0]*F[0]) )+ (1-xi)*gamma
